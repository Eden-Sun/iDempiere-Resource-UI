<template>
  <div class="space-y-4">
    <h2 class="text-base font-semibold text-slate-900">客戶維度查詢</h2>
    ...
    <div v-if="selectedBPartnerId && !loading && requests.length > 0" class="space-y-2">
      <h3 class="text-sm font-medium text-slate-700">諮詢歷史記錄（{{ requests.length }} 筆）</h3>
    ...
    <div v-if="selectedBPartnerId && !loading && requests.length === 0" class="rounded-lg border border-slate-200 bg-slate-50 p-8 text-center">
      <p class="text-sm text-slate-600">此客戶尚無諮詢記錄</p>
    </div>
...
async function loadCustomerRequests() {
  if (!auth.token.value || !selectedBPartnerId.value) return
