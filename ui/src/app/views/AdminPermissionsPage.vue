<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold">權限管理</h1>
        <p class="text-sm text-slate-500">設定使用者的選單存取權限</p>
      </div>
    </div>
      ...
      <div class="card-body">
        <div class="form-control max-w-xs">
          ...
          <select v-model="selectedUserId" class="select select-bordered select-sm" :disabled="loadingUsers">
            <option :value="null" disabled>請選擇使用者</option>
    ...
    <div v-if="selectedUserId" class="card bg-base-100 shadow-sm">
      <div class="card-body">
        <h2 class="card-title text-base">選單權限</h2>
        ...
        <div v-else class="space-y-2">
          ...
          >
            <div>
              ...
              <div class="text-xs text-slate-500">{{ item.description || item.path }}</div>
            </div>
            <input
              type="checkbox"
...
import { listUsers, getUserMenuPermissions, setUserMenuPermission } from '../../features/permission/api'
...
async function toggleMenu(menuId: string, enabled: boolean) {
  if (!selectedUserId.value || !auth.token.value) return
  saving.value = true
  error.value = null
  try {
