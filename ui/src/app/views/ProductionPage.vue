  ...
  <div class="space-y-6">
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-lg font-semibold">療程單</h1>
          <p class="mt-1 text-sm text-slate-600">管理療程執行紀錄</p>
        </div>
      </div>
    </div>
        ...
        <table v-if="activeTab === 'pending'" class="w-full text-sm">
          ...
          <tbody class="divide-y divide-slate-200">
            ...
            <tr v-else-if="pendingOrders.length === 0">
              <td colspan="6" class="px-4 py-8 text-center text-slate-500">無待轉療程</td>
            </tr>
            <tr
            ...
            >
              ...
              <td class="px-4 py-3 text-slate-600">{{ order.C_BPartner_ID?.identifier || order.bpartnerName }}</td>
        ...
        <table v-else class="w-full text-sm">
          ...
          <tbody class="divide-y divide-slate-200">
            ...
            <tr v-else-if="createdProductions.length === 0">
              <td colspan="6" class="px-4 py-8 text-center text-slate-500">無療程單</td>
            </tr>
            <tr
            ...
            >
              <td class="px-4 py-3 font-medium text-slate-900">{{ prod.documentNo }}</td>
              <td class="px-4 py-3 text-slate-600">{{ prod.bpartnerName }}</td>
              <td class="px-4 py-3 text-slate-600">{{ prod.productName }}</td>
              <td class="px-4 py-3 text-slate-600">{{ prod.productionQty }}</td>
              <td class="px-4 py-3">
                <span
    ...
    <div v-if="mode === 'create'" class="space-y-6">
      <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-base font-semibold">從銷售訂單創建療程單</h2>
          <button
            type="button"
        ...
        <div class="border border-slate-200 rounded-lg p-4 mb-4">
          <h3 class="text-sm font-medium text-slate-700 mb-3">銷售訂單資訊</h3>
        ...
        <div v-else class="space-y-3">
          <h3 class="text-sm font-medium text-slate-700">選擇要轉療程的明細</h3>
        ...
        <div v-if="selectedLineIndices.length > 0" class="mt-6">
          <h3 class="text-sm font-medium text-slate-700 mb-3">預計執行日期</h3>
          <input
    ...
    <div v-if="mode === 'detail'" class="space-y-6">
      <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-base font-semibold">療程單詳情</h2>
          <button
            type="button"
            class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
            @click="backToList"
        ...
        <div class="border border-slate-200 rounded-lg p-4 mb-4">
          <h3 class="text-sm font-medium text-slate-700 mb-3">療程單資訊</h3>
        ...
        <div v-else>
          <h3 class="text-sm font-medium text-slate-700 mb-3">療程明細</h3>
            ...
            <table class="w-full text-sm">
              ...
              <tbody class="divide-y divide-slate-200">
                <tr v-for="line in productionLines" :key="line.id">
                  <td class="px-4 py-3 text-slate-900">{{ line.productName }}</td>
                  <td class="px-4 py-3 text-slate-600">{{ line.movementQty }}</td>
                  <td class="px-4 py-3">
                    <span
...
async function handleCreateProduction() {
  if (!auth.token.value || !selectedOrder.value) return
...
async function handleCompleteProduction() {
  if (!auth.token.value || !currentProduction.value) return
