    ...
    <div v-if="!loading && !error" class="flex gap-6 overflow-x-auto pb-4">
      ...
      >
        ...
        >
          ...
          >
            <div class="space-y-2">
              <h4 class="font-medium text-slate-900">{{ request.name || '未命名' }}</h4>
              <p class="text-sm text-slate-600">{{ request.bPartnerName || '未指定客戶' }}</p>
              <div class="flex items-center justify-between">
          ...
          <div v-if="getRequestsByStatus(status.name).length === 0" class="text-center text-slate-400">
            <div class="text-sm">無{{ status.name }}的請求</div>
          </div>
        </div>
      </div>
    </div>
...
async function onDrop(event: DragEvent, statusId: number) {
  ...
  if (!draggedRequest.value || !auth.token.value) return
  ...
  if (request.requestStatusName === newStatus.name) return
